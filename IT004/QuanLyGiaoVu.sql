...
WHERE EXISTS (
	SELECT *
	FROM GIANGDAY g2
	WHERE g2.MALOP = l.MALOP AND g2.MAGV = g.MAGV)
...
-- Cau 25 --
SELECT h.HO + ' ' + h.TEN 'Họ tên trưởng lớp thi không đạt quá 3 môn'
FROM HOCVIEN h, LOP l, KETQUATHI k 
WHERE h.MAHV = l.TRGLOP AND h.MAHV = k.MAHV AND k.KQUA = 'Khong Dat'
GROUP BY l.TRGLOP, h.HO, h.TEN 
HAVING COUNT(*) > 3
...
FROM
(
	SELECT k.MAMH, h.MaHV, h.HO + ' ' + h.TEN HOTEN, RANK() OVER (PARTITION BY MAMH ORDER BY MAX(DIEM) DESC) AS XepHang
	FROM HOCVIEN h, KETQUATHI k 
	WHERE h.MAHV = k.MAHV 
		AND k.LANTHI = (SELECT MAX(LANTHI) FROM KETQUATHI WHERE MAHV = h.MAHV GROUP BY MAHV)
